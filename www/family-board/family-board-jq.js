...
class FamilyBoardJQ extends HTMLElement {
    ...
    return {
      ...
      fc: {
        ...
        views: {
          ...
          agendaWeek: { allDaySlot: true, slotDuration: '00:30:00', minTime: '06:00:00', maxTime: '22:00:00' },
          agendaDay:  { allDaySlot: true, slotDuration: '00:30:00', minTime: '06:00:00', maxTime: '22:00:00' },
        },
      },
      layoutOptions: {
        hideAppHeader: true,
  ...
  async _ensureAssets() {
    ...
    if (PATHS.momentTzUrl && window.moment && !window.moment.tz) await this._loadScript(PATHS.momentTzUrl);
  ...
  _loadCss(href, intoShadow = false) {
    return new Promise((res, rej) => {
      if (intoShadow && this._styleHost) {
        if (
          [...this._styleHost.querySelectorAll('link[rel="stylesheet"]')]
            .some((l) => l.href === href)
        ) return res();
    ...
    card.innerHTML = `
      <div class="fb-layout">
        ...
        <header>
          ...
          <div id="section-pill" class="fb-pill">${(this._state.section ?? 'Calendar').toUpperCase()}</div>
        </header>
  ...
  _renderSidebar() {
    const $ = this.$;
    const ICON = { Calendar: 'mdi:calendar', Chores: 'mdi:broom', Lists: 'mdi:format-list-bulleted', Photos: 'mdi:image-multiple' };
    ...
    (this._config.sections ?? ['Calendar', 'Chores']).forEach((sec) => {
      const $btn = $(
        `<button class="sb-btn" title="${this._escapeAttr(sec)}" role="button" tabindex="0" aria-pressed="${this._state.section === sec}">
      ...
      $btn.on('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') activate(); });
  ...
  _renderChips() {
    ...
    people.forEach((p) => {
      ...
      const $chip = $(`
        <div class="chip ${isActive ? 'active' : ''}"
          ...
          <div class="bar" role="progressbar"
            ...
            <div class="bar-fill" id="chip-progress-${this._escapeAttr(p.key)}" style="transform:scaleX(0)"></div>
          </div>
        </div>
      `);
      ...
      $chip.on('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') activate(); });
  ...
  _renderMain() {
    ...
    if (this._state.section === 'Calendar') {
      $main.append('<div id="fc-wrap"><div id="fc"><div class="fb-skeleton" style="height:420px;margin:12px"></div></div><div id="fc-legend" class="fb-legend"></div></div>');
      ...
      if (!this._resizeObserver)
        this._resizeObserver = new ResizeObserver(() => {
          try { this.$('#fc').fullCalendar('option', 'height', 'auto'); } catch {}
        });
    ...
    if (this._state.section === 'Chores') {
      $main.append('<div class="main-pad"><div id="chores" class="fb-chores-grid"></div></div>');
      this._renderChores();
      return;
    }

    $main.append('<div class="main-pad"><ha-card><div style="padding:12px">Not implemented</div></ha-card></div>');
  }
  ...
  _updateChipCounts() {
    ...
    Object.entries(map).forEach(([k, v]) => {
      // numeric pill
      this.$(`#chip-today-${k}`).text(v > 0 ? String(v) : '0').attr('title', `${v} open tasks`);
      ...
      $bar.attr({ 'aria-valuemin': 0, 'aria-valuemax': 10, 'aria-valuenow': Math.min(v, 10), 'aria-label': `${k} open ${v}` });
    });
  }
  ...
  _initFullCalendar() {
    ...
    try { $fc.fullCalendar('destroy'); } catch {}
    ...
    $fc.fullCalendar({
      ...
      defaultView: isNarrow() ? 'agendaDay' : (fcCfg.defaultView ?? fcCfg.initialView ?? 'month'),
      timezone: tz,
      ...
      viewRender: () => {
        requestAnimationFrame(() => {
          try { $fc.fullCalendar('option', 'height', 'auto'); } catch {}
          try { $fc.fullCalendar('refetchEvents'); } catch {}
        });
      },
    });
  ...
  _refetchFullCalendar() {
    try { this.$('#fc').fullCalendar('refetchEvents'); } catch {}
  }
  ...
  _renderChores() {
    ...
    const html = filterKeys.map((k) => {
      ...
      items = items.sort((a, b) =>
        (a.due ?? '').localeCompare(b.due ?? '') || ((b.priority ?? 0) - (a.priority ?? 0))
      );
  ...
  _hideBanner() { this.$('#fb-banner').remove(); }
  ...
  _fatal(msg) {
    ...
    this._body.innerHTML = `<ha-card><div class="fb-error">${this._escapeHtml(msg)}</div></ha-card>`;
  }
  _diag(msg) { if (this._config.diagnostics?.enabled) console.log('[family-board-jq]', msg); }
